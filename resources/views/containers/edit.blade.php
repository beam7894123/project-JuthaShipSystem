...
@include('alert')
@section('content')
<form action="{{ route('containers.update', ['container' => $container]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
            ...
            <div id="ship_list" class="bg-white rounded-md max-w-full mx-auto mt-8 overflow-y-auto">
                ...
                <div class="p-4">
                    ...
                    <div class="grid grid-cols-2 items-center p-6 text-3xl bg-white overflow-hidden shadow-sm sm:rounded-lg hover:bg-[#c0cfff] transition duration-300 m-4 text-black">
                        ...
                        <x-text-input type="text" class="block p-4 bg-white border rounded-md focus:outline-none focus:ring-2 focus:ring-[#c0cfff] " placeholder="your company name" :value="old('company_name', $container->company_name)" />
